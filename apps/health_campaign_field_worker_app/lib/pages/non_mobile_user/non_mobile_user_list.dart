...
class _NonMobileUserListPageState
  ...
  void initState() {
    ...
    attendanceBloc = AttendanceBloc(
      ...
      attendanceDataRepository: AttendanceLocalRepository(
        sql,
        am_oplog.AttendanceOpLogManager(isar),
      ),
      attendanceLogDataRepository: AttendanceLogsLocalRepository(
        sql,
  ...
  Future<void> _loadEnrollmentStatus({List<String>? individualIds}) async {
    try {
      ...
      if (individualIds != null) {
        final unenrolled =
            individualIds.where((id) => id.isNotEmpty && !enrolledIds.contains(id)).toList();
      ...
      if (mounted) {
        setState(() {
          ...
          // The initial local-only pass (no IDs) leaves the flag false so
          // cards don't flash "Enroll Face" before the network check completes.
  ...
  Future<void> _verifyCoWorker(
    ...
    // Using context after await is unreliable — matches the pattern in
    // reverification_popup.dart (lines 371-373).
        ...
        builder: (_) => MultiBlocProvider(
          ...
          child: FaceAttendanceWrapper(
            ...
            onVerified: (confidence) async {
              ...
              // Await the pre-fetched location (already in progress).
                  ...
                  attendanceState.maybeWhen(
                    ...
                    registerLoaded: (registers, offset, limit) {
                      ...
                      if (ids.isNotEmpty) {
                        ...
                        if (mounted) setState(() => _enrollmentStatusLoaded = true);
                      }
                    },
                  );
                                ...
                                children: [
                                  ...
                                  ...List.generate(
                                    ...
                                    return NonMobileUserCard(
                                      ...
                                      isVerifiedThisCycle: coWorkersVerifiedThisCycleNotifier.value.contains(enrollmentId),
                                      isEnrollmentLoading: !_enrollmentStatusLoaded,
                                      onFaceEnroll: () async {
