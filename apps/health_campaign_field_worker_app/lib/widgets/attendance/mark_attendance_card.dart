...
class MarkAttendanceCard extends ResolvedFlowWidget {
  ...
  Widget buildResolved(
    ...
    return WidgetStateContext.reactive(context, (ctx, state) {
      ...
      // Filtering attendees and attendance logs
  ...
  Widget _attendeeCard(context, item, compositeKey, filterAttendanceLogs, json,
    ...
    final presentButtonLabel =
        components["presentButton"]?["label"] ?? 'PRESENT';
    final presentButtonPrefixIcon = components["presentButton"]?["prefixIcon"];
    final presentButtonProperties =
        components["presentButton"]?["properties"] as Map<String, dynamic>? ??
            {};
    final presentButtonColor = presentButtonProperties["color"] ?? 'green';

    final absentButtonLabel = components["absentButton"]?["label"] ?? 'ABSENT';
    final absentButtonPrefixIcon = components["absentButton"]?["prefixIcon"];
    final absentButtonProperties =
        components["absentButton"]?["properties"] as Map<String, dynamic>? ??
            {};
    final absentButtonColor = absentButtonProperties["color"] ?? 'red';

    final signatureButtonLabel =
        components["signatureButton"]?["label"] ?? 'SIGNATURE';
    final signatureButtonProperties =
        components["signatureButton"]?["properties"] as Map<String, dynamic>? ??
            {};
    final signatureButtonColor = signatureButtonProperties["color"] ?? 'green';
    final signatureButtonPrefixIcon =
        components["signatureButton"]?["prefixIcon"];
  ...
  double _getAttendanceLogsStatus(
    ...
    final hasMorningLog = filteredAttendanceLogs.any((element) {
    ...
    ;
    final hasEveningLog = filteredAttendanceLogs.any((element) {
      final elementType = element["type"]?.toString();
      final elementStatus = element["status"];
      if (elementStatus == null) return false;
      if (elementType == "EXIT") {
  ...
  bool _allowManualAttendance(
    ...
    if (selectedDate == null) {
      return true; // if no date selected, allow marking (probably in a scenario where user selects date inside popup, so we want to show buttons until date is selected
    }
